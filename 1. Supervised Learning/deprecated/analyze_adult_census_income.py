...
# Input Age, Sex, Hours per Week [0, 9, 12]
# Output Income > 50K? [14]
# 32561

def integrate_sex(sex):
    if sex == "Female": return 0
    elif sex == "Male": return 1
    else: return 2

def validate_income(income):
    if income == "<=50K": return 0
    elif income == ">50K": return 1
    else: return 2
...
with open("datasets/Adult_Census_Income.csv", "r", encoding="utf8") as file:
...
for record in csv_list:
    ...
    input_vector.append(integrate_sex(record[9]))
    input_vector.append(int(record[12]))
    output_vector = validate_income(record[14])
