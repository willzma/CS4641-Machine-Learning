...
# Input duration, bike_id [1, 8]
# Output subscription_type [9]
# 49999

def integrate_subscription(subscription):
    if subscription == "Customer": return 0
    elif subscription == "Subscriber": return 1
    else: return 2
...
with open("datasets/SF_Bay_Area_Bike_Share.csv", "r", encoding="utf8") as file:
...
for record in csv_list:
    input_vector = []
    input_vector.append(int(record[1]))
    input_vector.append(int(record[8]))
    output_vector = integrate_subscription(record[9])
    if (count <= 39999):
